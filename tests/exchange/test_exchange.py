...
def test_create_dry_run_order_limit_fill(default_conf, mocker, side, price, filled,
    ...
    assert 'closed' if filled else order['status'] == 'open'
    order_book_l2_usd.reset_mock()

    # fetch order again...
    order_closed = exchange.fetch_dry_run_order(order['id'])
    assert order_book_l2_usd.call_count == (0 if filled else 1)
    assert order_closed['status'] == ('closed' if filled else 'open')
    assert order_closed['filled'] == (1 if filled else 0)
